...
module Proj1 (feedback, initialGuess, nextGuess, GameState) where
    ...
    type CardAlt = String
    type Guess = [CardAlt]       -- Guess is list of cards
    ...
    feedback anwser guess = 
        (correctCards, lowerRank, correctRank, higherRank, correctSuit)
            where 
                ...
                correctRank = totalNumber - correctCards -
                    length (deleteFirstsBy (areTheSame rank) anwser guess)
                correctSuit = totalNumber - correctCards - 
                    length (deleteFirstsBy (areTheSame suit) anwser guess)
                lowerRank = length [card | card <- guess, card !! rank < (sortBy compare anwser) !! 0 !! rank]
                higherRank = length [card | card <- guess, card !! suit > (sortBy compare anwser) !! (length anwser - 1) !! suit]
    ------------------------------------------------------------------------------
    ...
    areTheSame :: Int -> CardAlt -> CardAlt -> Bool
